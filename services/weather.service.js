...
async function resolveLocationName(lat, lng, fallback) {
  try {
    const url =
    ...
    // Reverse geocoding is optional; the weather reading must still render.
  }
...
async function getWeatherForScreen(screen, fallbackCoords) {
  ...
  const fetchPromise = (async () => {
    try {
      ...
      const humidity = typeof body.current.relative_humidity_2m === 'number'
        ? Math.round(body.current.relative_humidity_2m)
        : null;
      const windKmh = typeof body.current.wind_speed_10m === 'number'
        ? Math.round(body.current.wind_speed_10m)
        : null;
      ...
      const data = {
        ...
        humidity,
        windKmh,
        locationName: await resolveLocationName(lat, lng, screen && screen.place),
        isDay,
        updatedAt: Date.now()
      };
