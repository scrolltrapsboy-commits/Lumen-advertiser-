...
const { resolveMediaUrl } = require('../services/media-url.util');
...
async function upload(req, res, next) {
  try {
    ...
    // Verify the file that multer just wrote is actually present on disk,
    // and log everything needed to debug a "media stored but not visible"
    // report without guessing - this is the single point where a stored
    // file becomes a public URL, so any storage/path mismatch surfaces
    // right here instead of silently 404ing later on the display.
    const absolutePath = path.join(UPLOAD_DIR, file.filename);
    const existsOnDisk = fs.existsSync(absolutePath);
    const publicUrl = resolveMediaUrl(ad.mediaUrl, req);
    ...
    if (!existsOnDisk) {
      console.error('[MEDIA STORED] FILE MISSING ON DISK IMMEDIATELY AFTER UPLOAD - storage/UPLOAD_DIR misconfiguration', { absolutePath });
    }

    res.status(201).json({ ok: true, ad: { ...ad, mediaUrl: publicUrl } });
  } catch (err) {
...
async function display(req, res, next) {
  try {
    ...
    const rawAds = await adService.listPlayable(screenId);
    const settings = await settingsService.get();
    const timezone = settings.timezone || 'Asia/Kolkata';
    const isOpen = screen.activeState !== 'disabled' && isWithinOperatingHours(screen.openTime, screen.closeTime, timezone);

    // ONE canonical resolver, applied here so every consumer of this
    // endpoint (big display, small network preview, TV preview) receives
    // the exact same resolved http(s) mediaUrl - never a filesystem path,
    // never an unresolved relative path that a different origin could
    // mis-resolve.
    const ads = rawAds.map((ad) => ({ ...ad, mediaUrl: resolveMediaUrl(ad.mediaUrl, req) }));
    ads.forEach((ad) => console.log('[DISPLAY MEDIA]', { adId: ad.id, mediaUrl: ad.mediaUrl, mediaType: ad.mediaType }));
