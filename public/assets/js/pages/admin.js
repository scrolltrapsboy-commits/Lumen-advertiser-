  ...
  async function renderAdvertise() {
    ...
    content.innerHTML = `
      ...
      <div class="upload-grid mt-8">
        ...
        <div class="glass-card">
          ...
          <button class="btn btn-primary btn-block mt-6" id="submit-advertise" disabled>7. Upload Advertisement</button>
        </div>
        </div>

        <div class="upload-tv-col">
          <div class="glass-card glass-card--floating upload-tv-card">
            <div class="eyebrow" style="margin-bottom:12px;text-align:center;">This is what the selected display looks like</div>
            <div id="upload-tv-mount" class="small-display-tv-mount"></div>
          </div>
        </div>
      </div>
    `;

    // ONE reusable Small Display TV chassis + ONE live ScreenPreviewController
    // - the exact same two components used on the Advertiser upload page
    // (see small-display-tv.js / screen-preview.js). Admin can target
    // several/all screens at once (unlike the single-screen Advertiser
    // flow), so the preview represents whichever screen is FIRST in the
    // current target selection (see updatePreviewScreen(), called from
    // renderScreenTargeting() below on every targeting change).
    const tv = mountSmallDisplayTV(qs('#upload-tv-mount'));
    const screenPreview = createScreenPreview(tv.screenEl);
    screenPreview.setScreen(null);
    function updatePreviewScreen() {
      let id = null;
      if (state.screenIds === 'all') {
        const first = state.screens.find(s => s.activeState === 'active');
        id = first ? first.id : null;
      } else if (Array.isArray(state.screenIds) && state.screenIds.length) {
        id = state.screenIds[0];
      }
      screenPreview.setScreen(id);
    }
    ...
    qsa('.type-option').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        state.file = null;
        state.fileURL = null;
        screenPreview.setPendingMedia(null);
        updatePrice();
        updateSubmitState();
      });
    });
    ...
    async function handleFile(file) {
      ...
      screenPreview.setPendingMedia({ url: processedUrl, type: validation.type });
      updateSubmitState();
      showToast({ type: 'success', title: 'File ready', message: 'Preview updated above.' });
    }
