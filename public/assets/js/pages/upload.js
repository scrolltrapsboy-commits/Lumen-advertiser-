...
(async function init() {
    ...
    <div class="upload-grid mt-8">
      <div class="upload-form-col">
        <div class="glass-card">
          ...
          <button class="btn btn-primary btn-block mt-6" id="submit-upload" disabled>7. Upload Advertisement</button>
        </div>
      </div>
  ...
  // - the exact same two components used on the Admin upload page, so
  // there is a single source of truth for both (see small-display-tv.js /
  // screen-preview.js). Shows the REAL current state of whichever screen
  // is selected below; while a not-yet-uploaded file is selected, shows
  // that file instead (see handleFile()/state.screenId change below) -
  // this never touches the actual live display until submit succeeds.
  ...
  try {
    ...
    if (screens.length) screenPreview.setScreen(screens[0].id);
  } catch (err) {
