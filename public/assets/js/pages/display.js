...
(async function init() {
  ...
  const clockWeatherState = {
    ...
    // Owns the shared clock-weather overlay instance (see
    // clock-weather-overlay.js) for whichever screen is currently
    // mounted - destroyed/recreated by initClockAndWeather() below on
    // every mountPlayerShell() call, since mountPlayerShell replaces the
    // container element that the previous instance was rendered into.
    overlay: null,
  ...
  async function fetchWeatherOnce(screenId, isGeoRetry = false) {
    try {
      ...
      if (data && data.ok && data.weather) {
        ...
        if (clockWeatherState.overlay) clockWeatherState.overlay.renderWeather(data.weather);
      ...
      if (clockWeatherState.overlay) clockWeatherState.overlay.renderWeather(clockWeatherState.lastGoodWeather);
    } catch (err) {
  ...
  function initClockAndWeather(screen) {
    if (clockWeatherState.overlay) {
      clockWeatherState.overlay.destroy();
      clockWeatherState.overlay = null;
    }
    const clockContainer = document.getElementById('player-liquid-clock');
    if (clockContainer) {
      clockWeatherState.overlay = mountClockWeatherOverlay(clockContainer, { detailed: true });
    }
  ...
  function mountPlayerShell(config, screen) {
    ...
    playerView.innerHTML = `
      <div class="player-shell" id="player-shell-root">
        ...
        <div class="player-daily-quote" aria-label="Daily quote">${escapeHTML(dailyQuote())}</div>
        ...
        <div class="player-liquid-clock" id="player-liquid-clock"></div>
...
<div class="player-qr" id="player-qr">
  <div class="player-qr-card">
    ...
    if (walkerEl && QR_VIDEO_DEMO_MODE) initQRVideoWalker(walkerEl);
  }
