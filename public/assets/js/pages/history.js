...
(async function init() {
  ...
  async function render(force = false) {
    ...
    try {
      ...
      console.error('[History] Failed to load advertisements:', err);
      ...
      if (empty) {
        ...
        empty.innerHTML = `<div class="glass-card empty-state"><h3>Could not load your advertisements</h3><p>Check your connection and try again.</p><button type="button" class="btn btn-secondary mt-4" id="history-retry">Retry</button></div>`;
        const retryBtn = document.getElementById('history-retry');
        if (retryBtn) retryBtn.addEventListener('click', () => render(true));
      }
      return;
    }
