...
export function createLiquidDropdown(options = {}) {
  ...
  const optionsHtml = dropdownOptions.map(opt => {
    ...
    // renderOption returns trusted internal markup (e.g. the screen-status
    // badge template in upload.js) - escaping it here turned that markup
    // into inert literal text instead of a rendered badge, which combined
    // with this item's own white-space:nowrap/overflow:hidden is exactly
    // why a status badge placed after a long screen name could get clipped
    // away on narrow screens: it was never a real element to begin with,
    // just characters at the tail end of one long escaped string. Only the
    // plain-string fallback (opt.label, which can contain real user data)
    // still needs escaping.
    const labelHtml = renderOption ? label : escapeHtml(label);

    return `
      <li class="lg-dropdown__item ${renderOption ? 'lg-dropdown__item--rich' : ''} ${isSelected ? 'lg-dropdown__item--selected' : ''} ${isDisabled ? 'lg-dropdown__item--disabled' : ''}"
          data-value="${escapeHtml(opt.value)}"
          role="option"
          aria-selected="${isSelected}"
          ${isDisabled ? 'aria-disabled="true"' : ''}
          tabindex="${isDisabled ? '-1' : '0'}">
        <span class="lg-dropdown__item-text">${labelHtml}</span>
        ${checkIcon}
      </li>
    `;
  }).join('');
