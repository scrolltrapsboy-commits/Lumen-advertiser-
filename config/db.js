...
// `build` is stamped once when this server process boots (i.e. on every
// deploy/restart) and never changes again until the next boot. Long-running
// kiosk display tabs already poll /api/meta every couple of seconds via
// watchLive() for ads/screens changes - display.js also watches this key and
// does a full page reload when it changes, so a shipped code fix actually
// reaches an always-on kiosk tab instead of silently continuing to run
// whatever JavaScript happened to already be loaded in memory from before
// the deploy. Redeploying the server does not by itself update code already
// executing in a browser tab; this is what closes that gap.
versions.build = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
